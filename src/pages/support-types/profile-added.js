...
import IconCircleCheck from "../../svgs/icons/icon-check-circle.svg";
import Button from "../../components/button";
import { Link } from "gatsby";

const SupportTypesProfileAddedPage = props => {
  const cards = [
    {
    ...
    <Layout activePage={props.uri} showNotification>
      <section className={style.supportTypesPage}>
        ...
        <div className={style.supportCardContainer}>
          {cards.map(({ cardIcon, title }, index) => (
            <div className={`${style.supportCard} ${style.supportCardActive}`} key={title}>
              <LinkCard supportType>
                {cardIcon}
                <h4>{title}</h4>
                <div className={style.buttonContainer}>
                  <Link to="/support-types/alert">Learn More</Link>
...
export default SupportTypesProfileAddedPage;
